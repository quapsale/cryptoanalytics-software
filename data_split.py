"""
File: data_split.py
Description: Train-valid split for model pre-training.
...
# Parser for CLI
parser = argparse.ArgumentParser(description='split coin price dataset for model pre-training')
...
# No data split
if not (args.train and args.valid):
    print('Missing arguments: --train and --valid are required!')
...
if not args.filenames:
    ...
    if len(args.filenames) != 2:
        print('Invalid arguments provided: number of filenames is not 2!')
        sys.exit(1)
...
print({'--data': data, '--variable': args.variable, '--train': tr,
       '--valid': vd, '--path': args.path,  '--filenames': [filename_1, filename_2]})
