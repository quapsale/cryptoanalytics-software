...
parser.add_argument('-ft', '--features', type=str, nargs=1,
                    help='path for json with feature variable list')
...
# Print args
print({'--train': train, '--valid': valid, '--target': target, '--features': feat,
            ...
            try:
                ...
                try:
                    ...
                    if mdl == 'gru':
                        ...
                        try:
                            ...
                            model = GRU(n_features=len(features), hidden_units=hidden_units, n_layers=n_layers,
                                        lr=learning_rate)
                            trainer = pl.Trainer(callbacks=[early_stopping], max_epochs=n_epochs,
                                                 gpus=gpus)

                            # Fit
                            trainer.fit(model, train_loader, validation_loader)
                            torch.save(model.state_dict(), os.path.join(args.path, filename + '.pth'))
                    ...
                    # XGBoost
                        ...
                        x_train = train_scaled.loc[:, train_scaled.columns != target]
                        y_train = train_scaled[target]
                        x_valid = valid_scaled.loc[:, valid_scaled.columns != target]
                        y_valid = valid_scaled[target]
